...
class Check {
	...
	private static function run() {
		...
		if ( 'application/gzip' !== mime_content_type( self::$params['package'] ) ) {
			throw new \Exception( __( 'Your backup file requires Everest Backup version 1.1.7 or earlier, aborting restore.', 'everest-backup' ) );
		}
		...
		if ( empty( $stats ) ) {
			throw new \Exception( __( 'Uploaded backup is either corrupted or not supported. Metadata missing, aborting restore.', 'everest-backup' ) );
		}
		...
		if ( ! everest_backup_is_space_available( WP_CONTENT_DIR, absint( $stats['size'] ) ) ) {
			throw new \Exception( __( 'Required space not available, aborting restore.', 'everest-backup' ) );
		}
		...
		$tags = $archiver->get_metadata( 'tags' );
		...
		if ( ! empty( $debug['throw_error'] ) ) {
			throw new \Exception( __( 'This error is generated manually using Everest Backup debugger.', 'everest-backup' ) );
		}
